...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {editingMeal && <MealActionSheet day={week.find((day) => day.date === editingMeal.date)!} mealType={editingMeal.mealType} recipes={recipeItems} hasOverride={mealOverrides.some((item) => item.date === editingMeal.date && (item.mealType || 'dinner') === editingMeal.mealType)} onClose={() => setEditingMeal(null)} onSave={(kind, recipeId, servings) => changeMeal(editingMeal.date, editingMeal.mealType, kind, recipeId, servings)} onReset={() => resetMeal(editingMeal.date, editingMeal.mealType)} />}
  </main>;
}
...
function CalendarView({ events, trips, week, recipes, completions, householdId, editMeal, onChanged, onDeleted, notify }: { events: ScheduleException[]; trips: ShoppingTrip[]; week: PlanningDay[]; recipes: Recipe[]; completions: MealCompletion[]; householdId?: string; editMeal: (date: string, mealType: MealTypeKey) => void; onChanged: (changed: ScheduleException) => void; onDeleted: (id: string) => void; notify: (message: string) => void }) {
...
function AiRecipeStudio({ goals, proposals, inventory, week, householdId, onGoals, onApproved, notify }: { goals: HouseholdFoodGoals; proposals: AiRecipeProposal[]; inventory: InventoryItem[]; week: PlanningDay[]; householdId?: string; onGoals: (goals: HouseholdFoodGoals) => void; onApproved: (recipe: Recipe) => void; notify: (message: string) => void }) {
...
function RecipesView({ recipes, inventory, week, householdId, onUpdated, onCreated, onDeleted, notify }: { recipes: Recipe[]; inventory: InventoryItem[]; week: PlanningDay[]; householdId?: string; onUpdated: (recipe: Recipe) => void; onCreated: (recipe: Recipe) => void; onDeleted: (id: string) => void; notify: (message: string) => void }) {
  ...
  return <section className="recipes-page">
    <AiRecipeStudio goals={goals} proposals={proposals} inventory={inventory} week={week} householdId={householdId} onGoals={setGoals} onApproved={onCreated} notify={notify} />
...
function MealActionSheet({ day, mealType, recipes, hasOverride, onClose, onSave, onReset }: { day: PlanningDay; mealType: MealTypeKey; recipes: Recipe[]; hasOverride: boolean; onClose: () => void; onSave: (kind: DinnerOverrideKind, recipeId: string | null, servings: number) => void; onReset: () => void }) {
...
function RoutineEditor({ foods, recipes, householdId, onClose, onChanged, onDeleted, notify }: { foods: RecurringFood[]; recipes: Recipe[]; householdId?: string; onClose: () => void; onChanged: (food: RecurringFood) => void; onDeleted: (id: string) => void; notify: (message: string) => void }) {
  ...
  const save = async () => {
    ...
    const ingredient = draft.kind === 'item' && draft.ingredient ? { ...draft.ingredient, name, itemId: name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') } : null;
