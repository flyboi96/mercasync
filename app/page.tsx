...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {active === 'Plan' && <PlanView items={displayItems} inventory={inventory} week={week} profiles={recurringProfiles} dinnerTarget={dinnerTarget} setDinnerTarget={changeDinnerTarget} startReset={() => setResetOpen(true)} editRoutines={() => setRoutinesOpen(true)} editMeal={(date) => setEditingMeal({ date, mealType: 'dinner' })} open={setActive} aiBrief={aiBrief} aiRequest={aiRequest} householdId={auth.session?.householdId} notify={notify} />}
      {active === 'Calendar' && <CalendarView events={events} trips={displayedTrips} week={week} recipes={recipeItems} inventory={inventory} completions={mealCompletions} householdId={auth.session?.householdId} editMeal={(date, mealType) => setEditingMeal({ date, mealType })} onChanged={(changed) => setEvents((current) => [...current.filter((event) => event.id !== changed.id), changed].sort((a, b) => a.date.localeCompare(b.date)))} onDeleted={(id) => setEvents((current) => current.filter((event) => event.id !== id))} notify={notify} />}
...
function PlanView({ items, inventory, week, profiles, dinnerTarget, setDinnerTarget, startReset, editRoutines, editMeal, open, aiBrief, aiRequest, householdId, notify }: { items: Grocery[]; inventory: InventoryItem[]; week: PlanningDay[]; profiles: RecurringFood[]; dinnerTarget: number; setDinnerTarget: (target: number) => void; startReset: () => void; editRoutines: () => void; editMeal: (date: string) => void; open: (tab: string) => void; aiBrief: AiPlanningBrief | null; aiRequest: AiGenerationRequest | null; householdId?: string; notify: (message: string) => void }) {
...
function CalendarView({ events, trips, week, recipes, inventory, completions, householdId, editMeal, onChanged, onDeleted, notify }: { events: ScheduleException[]; trips: ShoppingTrip[]; week: PlanningDay[]; recipes: Recipe[]; inventory: InventoryItem[]; completions: MealCompletion[]; householdId?: string; editMeal: (date: string, mealType: MealTypeKey) => void; onChanged: (changed: ScheduleException) => void; onDeleted: (id: string) => void; notify: (message: string) => void }) {
  ...
  return <section className="calendar-page">
    ...
    {view === 'work' ? <>
      ...
      <div className="meal-week-calendar">{week.map((day) => {
        ...
        return <article className={day.isToday ? 'meal-day today' : 'meal-day'} key={day.date}><header><span>{day.dayLabel}</span><strong>{day.dateLabel}</strong></header><div className="meal-slot lunch-slot"><small>LUNCH · {day.lunch.effort} · {day.lunch.servings} SERVINGS</small><button className="meal-title-button" onClick={() => editMeal(day.date, 'lunch')}>{day.lunch.servings ? day.lunch.title : 'Lunch off'}</button><MealStatusControls status={lunchStatus} disabled={!day.lunch.recipeId} onChange={(status) => reconcileMeal(day, 'lunch', status)} /></div><div className={`meal-slot ${day.meal.tone}`}><small>DINNER · {day.meal.effort.toUpperCase()} · {day.meal.servings} SERVINGS</small><button className="meal-title-button" onClick={() => editMeal(day.date, 'dinner')}>{day.meal.title}</button><MealStatusControls status={dinnerStatus} disabled={!day.meal.recipeId} onChange={(status) => reconcileMeal(day, 'dinner', status)} /></div>{dayTrips.map((trip) => <span className="shopping-chip" key={trip.id}>{trip.store}</span>)}</article>;
      })}</div>
    </>}
