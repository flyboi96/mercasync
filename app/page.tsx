...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {active === 'Recipes' && <RecipesView recipes={recipeItems} householdId={auth.session?.householdId} onUpdated={(updated) => setRecipeItems((current) => current.map((recipe) => recipe.id === updated.id ? updated : recipe))} onCreated={(created) => setRecipeItems((current) => [...current.filter((recipe) => recipe.id !== created.id), created].sort((a, b) => a.name.localeCompare(b.name)))} onDeleted={(id) => setRecipeItems((current) => current.filter((recipe) => recipe.id !== id))} notify={notify} />}
...
function RecipesView({ recipes, householdId, onUpdated, onCreated, onDeleted, notify }: { recipes: Recipe[]; householdId?: string; onUpdated: (recipe: Recipe) => void; onCreated: (recipe: Recipe) => void; onDeleted: (id: string) => void; notify: (message: string) => void }) {
